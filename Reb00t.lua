--[[
    REBOOT — Total Roblox Drama
    Gen2 UI migration of the existing script source.
    Gameplay/feature logic is preserved; only the UI layer and branding are migrated.
    Made by travi.
]]

-- Executor compatibility bootstrap preserved from the source.
...
-- Rayfield Gen2
local __Gen2 = loadstring(game:HttpGet("https://sirius.menu/gen2"))()

local __RebootFlags = {}
local __RebootElements = {}
local __RebootWindows = {}
...
local function __flagProxy(flag, kind, element, initial, extra)
    if not flag or flag == "" then return element end
    local proxy = {
        Type = kind,
        _element = element,
        CurrentValue = initial,
        CurrentOption = (type(initial) == "table") and initial or nil,
        CurrentKeybind = (kind == "Keybind") and initial or nil,
        Color = (kind == "ColorPicker") and initial or nil,
    }
    if extra then
        for k,v in pairs(extra) do proxy[k] = v end
    end
    function proxy:Set(value, skipCallback)
        if proxy._element and proxy._element.Set then
            pcall(function() proxy._element:Set(value, skipCallback) end)
        end
        if kind == "Dropdown" then
            proxy.CurrentOption = type(value) == "table" and value or {value}
            proxy.CurrentValue = proxy.CurrentOption
        elseif kind == "Keybind" then
            proxy.CurrentKeybind = value
    ...
    function proxy:Refresh(options)
        if proxy._element and proxy._element.Refresh then
            pcall(function() proxy._element:Refresh(options) end)
        end
    end
    setmetatable(proxy, {
        __index = function(t,k)
            if k == "CurrentValue" or k == "CurrentOption" or k == "CurrentKeybind" or k == "Color" then
                return rawget(t,k)
            end
        end
    })
    __RebootFlags[flag] = proxy
    __RebootElements[flag] = proxy
    return proxy
end

local function __normalizeName(cfg)
    cfg = __copy(cfg)
    cfg.name = cfg.name or cfg.Name
    cfg.icon = cfg.icon or cfg.Icon
    cfg.flag = cfg.flag or cfg.Flag
    return cfg
end

local function __wrapElement(element, cfg, kind, initial)
    cfg = cfg or {}
    local flag = cfg.flag or cfg.Flag
    local proxy
    local original = cfg.callback or cfg.Callback
    local wrapped = function(value, ...)
        ...
        if original then return original(value, ...) end
    end
    -- Gen2 element callbacks are already installed when created; the caller supplies wrapped config.
    proxy = __flagProxy(flag, kind, element, initial)
    return proxy
end

local function __makeTabAdapter(rawTab)
    local tab = {}

    function tab:CreateButton(cfg)
        cfg = __normalizeName(cfg)
        local e = rawTab:CreateButton({name=cfg.name, icon=cfg.icon, description=cfg.Description or cfg.description, callback=cfg.callback or cfg.Callback})
        return e
    end

    function tab:CreateToggle(cfg)
        cfg = __normalizeName(cfg)
        local initial = cfg.CurrentValue
        local proxy
        local cb = cfg.callback or cfg.Callback
        local e = rawTab:CreateToggle({
            name=cfg.name, icon=cfg.icon, description=cfg.Description or cfg.description,
            flag=cfg.flag, value=initial == nil and false or initial,
            callback=function(v)
                if proxy then proxy.CurrentValue = v end
                if cb then return cb(v) end
            end,
        })
        proxy = __wrapElement(e, cfg, "Toggle", initial == nil and false or initial)
        return proxy
    end
    ...
    function tab:CreateSlider(cfg)
        ...
        local proxy
        local cb = cfg.callback or cfg.Callback
        local e = rawTab:CreateSlider({
            name=cfg.name, icon=cfg.icon, description=cfg.Description or cfg.description,
            flag=cfg.flag, range=cfg.Range, increment=cfg.Increment, suffix=cfg.Suffix,
            value=initial,
            callback=function(v)
                if proxy then proxy.CurrentValue = v end
                if cb then return cb(v) end
        ...
        proxy = __wrapElement(e, cfg, "Slider", initial)
        return proxy
    end

    function tab:CreateDropdown(cfg)
        cfg = __normalizeName(cfg)
        local initial = cfg.CurrentOption
        if initial == nil then initial = cfg.CurrentValue end
        local proxy
        local cb = cfg.callback or cfg.Callback
        local e = rawTab:CreateDropdown({
            name=cfg.name, icon=cfg.icon, description=cfg.Description or cfg.description,
            flag=cfg.flag, options=cfg.Options or {}, value=initial or {},
            multiSelect=cfg.MultipleOptions == true,
            placeholder=cfg.PlaceholderText,
            callback=function(v)
                if proxy then
                    proxy.CurrentOption = type(v) == "table" and v or {v}
                    proxy.CurrentValue = proxy.CurrentOption
                end
                if cb then return cb(v) end
            end,
        })
        proxy = __wrapElement(e, cfg, "Dropdown", initial or {})
        return proxy
    end

    function tab:CreateInput(cfg)
        cfg = __normalizeName(cfg)
        local initial = cfg.CurrentValue or ""
        local proxy
        local cb = cfg.callback or cfg.Callback
        local e = rawTab:CreateInput({
            name=cfg.name, icon=cfg.icon, description=cfg.Description or cfg.description,
            flag=cfg.flag, value=initial, placeholder=cfg.PlaceholderText or "",
            numeric=cfg.Numeric == true, clearOnFocus=false,
            callback=function(v)
                if proxy then proxy.CurrentValue = v end
                if cb then return cb(v) end
            end,
        })
        proxy = __wrapElement(e, cfg, "Input", initial)
        return proxy
    end

    function tab:CreateKeybind(cfg)
        cfg = __normalizeName(cfg)
        local initial = cfg.CurrentKeybind
        local proxy
        local cb = cfg.callback or cfg.Callback
        local e = rawTab:CreateKeybind({
            name=cfg.name, icon=cfg.icon, description=cfg.Description or cfg.description,
            flag=cfg.flag, value=initial, hold=cfg.HoldToInteract == true,
            callback=function(v)
                if proxy then
                    proxy.CurrentKeybind = v
                    proxy.CurrentValue = v
                end
                if cb then return cb(v) end
            end,
        })
        proxy = __wrapElement(e, cfg, "Keybind", initial)
        return proxy
    end

    function tab:CreateColorPicker(cfg)
        cfg = __normalizeName(cfg)
        local initial = cfg.Color or Color3.fromRGB(255,255,255)
        local proxy
        local cb = cfg.callback or cfg.Callback
        local e = rawTab:CreateColorPicker({
            name=cfg.name, icon=cfg.icon, description=cfg.Description or cfg.description,
            flag=cfg.flag, color=initial,
            callback=function(v, alpha)
                if proxy then proxy.Color = v; proxy.CurrentValue = v end
                if cb then return cb(v, alpha) end
            end,
        })
        proxy = __wrapElement(e, cfg, "ColorPicker", initial)
        proxy.Color = initial
        return proxy
    end

    function tab:CreateSection(title)
        local name = type(title) == "table" and (title.name or title.Name or "") or tostring(title or "")
        return rawTab:CreateSection({name=name})
    end

    function tab:CreateDivider()
        -- Gen2 has no legacy Divider primitive; an empty section preserves spacing without breaking callbacks.
        return rawTab:CreateSection({name=" "})
    end

    function tab:CreateLabel(text, icon, color, ignoreTheme)
        -- Gen2's compact section is the closest native text-only primitive.
        return rawTab:CreateSection({name=tostring(text or "")})
    end
...
local Reboot = {}
Reboot.Flags = __RebootFlags
Reboot._elements = __RebootElements

function Reboot:CreateWindow(cfg)
    cfg = cfg or {}
    local theme = cfg.Theme or cfg.theme or "Default"
    local raw = __Gen2:CreateWindow({
        name = "REBOOT",
        subtitle = "Total Roblox Drama • " .. tostring(cfg.LoadingTitle or cfg.subtitle or "Reboot"),
        theme = theme,
    })
    local window = {
        _raw = raw,
    ...
    function window:CreateTab(name, icon)
        ...
        if type(name) == "table" then
            props = __copy(name)
            props.name = props.name or props.Name
            props.icon = props.icon or props.Icon
        else
            props = {name=tostring(name), icon=icon}
        end
        return __makeTabAdapter(raw:CreateTab({name=props.name, icon=props.icon}))
    end
    window.ModifyTheme = function(themeName)
        return raw:ChangeTheme(themeName)
    end
    ...
    function window:Notify(data)
        data = data or {}
        return raw:Notify({title=data.Title or data.title, content=data.Content or data.content, icon=data.Image or data.icon, duration=data.Duration or data.duration})
    end
    function window:Get(flag)
        return raw:Get(flag)
    end
    function window:Set(flag, value)
        return raw:Set(flag, value)
    end
    function window:Save(name) return raw:Save(name) end
    function window:Load(name) return raw:Load(name) end
    function window:ListConfigs() return raw:ListConfigs() end
    function window:DeleteConfig(name) return raw:DeleteConfig(name) end
    function window:Destroy() return raw:Unload() end
    function window:Unload() return raw:Unload() end
    function window:Hide() return raw:Hide() end
    function window:Show() return raw:Show() end
    function window:ToggleHide() return raw:ToggleHide() end
    function window:ToggleMinimise() return raw:ToggleMinimise() end
    __RebootWindows[#__RebootWindows+1] = window
    return window
end

function Reboot:Notify(data)
    for i = #__RebootWindows, 1, -1 do
        local w = __RebootWindows[i]
        if w and w._raw and not w._raw.unloaded then
            return w:Notify(data)
        end
    end
end

-- Keep the old variable name so every existing feature callback continues to use it unchanged.
local Rayfield = Reboot

-- Compatibility aliases for the preserved configuration helpers.
local _RebootHttp = game:GetService("HttpService")
local _Reboot_Elements = __RebootElements
...
-- Reboot unified font selector: 10+ built-in fonts, all local and dependency-free.
local __RebootFonts = {
    "Gotham", "GothamBold", "GothamMedium", "SourceSans", "SourceSansBold", "SourceSansSemibold",
    "Arial", "ArialBold", "Cartoon", "Code", "Fantasy", "SciFi", "Arcade", "Bangers", "Oswald",
    "Roboto", "RobotoMono", "RobotoCondensed"
}
local __RebootFontMap = {}
for _, fontName in ipairs(__RebootFonts) do
    local ok, fontValue = pcall(function() return Enum.Font[fontName] end)
    if ok and fontValue then __RebootFontMap[fontName] = fontValue end
end
local function __RebootApplyFont(fontName)
    local selected = __RebootFontMap[fontName]
    if not selected then return end
    _G.SelectedFont = selected
    for _, gui in ipairs(game:GetDescendants()) do
        pcall(function()
            if gui:IsA("TextLabel") or gui:IsA("TextButton") or gui:IsA("TextBox") then
                if not gui:FindFirstAncestor("RobloxGui") then gui.Font = selected end
            end
        end)
    end
end
local function __RebootAddFontDropdown(tab)
    if not tab then return end
    local options = {}
    for _, name in ipairs(__RebootFonts) do
        if __RebootFontMap[name] then table.insert(options, name) end
    end
    tab:CreateDropdown({
        Name = "Font", Options = options, CurrentOption = {options[1] or "Gotham"},
        MultipleOptions = false, Flag = "RebootFont",
        Callback = function(option)
            local selected = type(option) == "table" and option[1] or option
            if selected then __RebootApplyFont(selected) end
        end,
    })
end
...
if isLobby then
    ...
    __RebootAddFontDropdown(ClientTab)
    _G.SelectedFont = nil
...
-- Reboot Chat controls
local __RebootChatMessage = ""
local __RebootChatDelay = 0.5
local __RebootChatSpamming = false
local __RebootChatRemote = nil
local function __RebootFindChatRemote()
    local rs = game:GetService("ReplicatedStorage")
    local candidates = {rs:FindFirstChild("SayMessageRequest"), rs:FindFirstChild("Chat"), rs:FindFirstChild("RemoteEvent"), rs:FindFirstChild("DefaultChatSystemChatEvents")}
    for _, remote in ipairs(candidates) do
        if remote then
            if remote:IsA("RemoteEvent") or remote:IsA("RemoteFunction") then return remote end
            if remote.Name == "DefaultChatSystemChatEvents" then
...
__RebootChatRemote = __RebootFindChatRemote()
local function __RebootStopChatSpam() __RebootChatSpamming = false end
local function __RebootStartChatSpam()
    if __RebootChatSpamming or __RebootChatMessage == "" then return end
    __RebootChatSpamming = true
    task.spawn(function()
        while __RebootChatSpamming do
            local remote = __RebootChatRemote or __RebootFindChatRemote()
            if remote then pcall(function() remote:FireServer(__RebootChatMessage, "All") end) end
            task.wait(math.max(0.5, tonumber(__RebootChatDelay) or 0.5))
        end
    end)
end
if MainTab then
    MainTab:CreateSection("Chat")
    MainTab:CreateInput({Name="Chat Message", PlaceholderText="Put text here...", RemoveTextAfterFocusLost=false, Callback=function(text) __RebootChatMessage=tostring(text or ""); if __RebootChatMessage == "" then __RebootStopChatSpam() elseif __RebootChatSpamming == false then end end})
    MainTab:CreateSlider({Name="Chat Delay", Range={0.5,5}, Increment=0.1, Suffix="sec", CurrentValue=0.5, Flag="RebootChatDelay", Callback=function(value) __RebootChatDelay=value end})
    MainTab:CreateToggle({Name="Chat Spam", CurrentValue=false, Flag="RebootChatSpam", Callback=function(value) if value then __RebootStartChatSpam() else __RebootChatStopChatSpam() end end})
end
...
local _dwInitialList = Reboot_ListConfigs()
local _dwConfigDropdown = SettingsTab:CreateDropdown({
    ...
    Callback = function(Options)
        if Options[1] and Options[1] ~= "(no saved configs)" then
            ...
            Reboot_LoadConfig(_dwSelectedConfig)
...
SettingsTab:CreateButton({
    ...
    Callback = function()
        ...
        local path = "Reboot/Configs/" .. _dwSelectedConfig .. ".json"
        local deleted = false
        pcall(function()
...
MainTab:CreateInput({
    Name = "Webhook URL",
    PlaceholderText = "Webhook URL disabled",
    RemoveTextAfterFocusLost = false,
...
do
    ...
    originally you would be able to use any accessory etc
...
-- autoplay
-- if u want better, prototype route or MAKE YOUR OWN!
...
local TAS_RouteSources = {
    ...
    ["Cave Chaos Clip"] = {type = "url", source = "https://raw.githubusercontent.com/reboot/tas-skid-skid-skid/refs/heads/main/Cave%20Chaos%20Clip.json"},
...
Rayfield:Notify({Title = "Reboot", Content = "All systems loaded.", Duration = 3})
