--// HEAVELY HUB
--// Total Roblox Drama Project
--// Credits: travi & ChatGPT
...
local _HeavelyHubHttp = game:GetService("HttpService")
local _HeavelyHub_Elements = {}
...
local function HeavelyHub_EnsureFolders()
    print("ensurin folders")
    pcall(function()
        if not isfolder("HeavelyHub") then makefolder("HeavelyHub") end
        if not isfolder("HeavelyHub/Configs") then makefolder("HeavelyHub/Configs") end
    end)
end

local function HeavelyHub_ListConfigs()
    print("listing cfgs")
    local names = {}
    pcall(function()
        HeavelyHub_EnsureFolders()
        local files = (listfiles and listfiles("HeavelyHub/Configs")) or {}
...
local _HeavelyHub_SkipFlags = { HeavelyHubConfigDropdown = true }

local function HeavelyHub_PackColor(c)
    return {R = math.floor(c.R * 255 + 0.5), G = math.floor(c.G * 255 + 0.5), B = math.floor(c.B * 255 + 0.5)}
end

local function HeavelyHub_UnpackColor(t)
    return Color3.fromRGB(t.R or 0, t.G or 0, t.B or 0)
end

local function HeavelyHub_SaveConfig(name)
    print("savin config")
    HeavelyHub_EnsureFolders()
    local data = {}
    for flag, obj in pairs(Rayfield.Flags) do
        if not _HeavelyHub_SkipFlags[flag] then
            pcall(function()
                if obj.Type == "ColorPicker" then
                    if obj.Color then
                        data[flag] = HeavelyHub_PackColor(obj.Color)
                    end
    ...
    local ok, json = pcall(function() return _HeavelyHubHttp:JSONEncode(data) end)
    if ok then
        pcall(function() writefile("HeavelyHub/Configs/" .. name .. ".json", json) end)
    end
end

local function HeavelyHub_LoadConfig(name)
    local path = "HeavelyHub/Configs/" .. name .. ".json"
    local exists = false
    pcall(function() exists = isfile(path) end)
    if not exists then return false end
    local ok, data = pcall(function()
        return _HeavelyHubHttp:JSONDecode(readfile(path))
    end)
    if ok and data then
                ...
                task.spawn(function()
                    pcall(function()
                        ...
                        if flagObj and flagObj.Type == "ColorPicker" and type(v) == "table" then
                            v = HeavelyHub_UnpackColor(v)
                        end
                        local el = _HeavelyHub_Elements[f]
...
local function HeavelyHub_PatchTab(tab)
    print("patching tab")
        ...
        if orig then
            tab[m] = function(s, cfg, ...)
                ...
                if cfg and type(cfg) == "table" and cfg.Flag and el then
                    _HeavelyHub_Elements[cfg.Flag] = el
                end
                return el
            end
        end
...
local function saveTheme(theme)
    pcall(function()
        if not isfolder("HeavelyHub") then makefolder("HeavelyHub") end
        writefile("HeavelyHub/Theme.txt", theme)
    end)
end
...
local function loadTheme()
    local theme = nil
    pcall(function()
        if isfile("HeavelyHub/Theme.txt") then
            local t = readfile("HeavelyHub/Theme.txt"):match("^%s*(.-)%s*$")
...
do
    ...
    _G._HeavelyHub_SetLedge = function(v)
        _LedgeEnabled    = v
        _FpsBounceEnabled = v
    end
...
if isLobby then
    local Window = Rayfield:CreateWindow({
        Name = "Heavely Hub - .gg/6PaSN7wepr",
        LoadingTitle = " Heavely Hub (" .. game_type .. ")",
        LoadingSubtitle = "made by travi and ChatGPT",
        ShowText = "Heavely Hub",
        Theme = chosen_theme,
        DisableRayfieldPrompts = true,
        DisableBuildWarnings = true,
        CustomUIStrings = {
            ["Rayfield"] = "Heavely Hub",
            ["Hide"] = "Hide",
            ["Show"] = "Show"
        },
        ConfigurationSaving = {
            Enabled = true,
            FolderName = "HeavelyHub",
            FileName = "Config"
        }
    })
    local InfoTab = Window:CreateTab("Info", "info")
    InfoTab:CreateSection("Heavely Hub")
...
local function equipGear(toolName)
    ...
    if not _gearFolder then warn("[HeavelyHub] Gear folder not found!") return end
    local _gearTool = _gearFolder:FindFirstChild(toolName)
    if not _gearTool then warn("[HeavelyHub] Tool not found: " .. toolName) return end
...
SettingsTab:CreateButton({
    ...
    Callback = function()
        ...
        local path = "HeavelyHub/Configs/" .. _dwSelectedConfig .. ".json"
        local deleted = false
        pcall(function()
...
do
    ...
    originally you would be able to use any accessory etc but the original implementation note was removed
    ...
    local function _csb_getSavedSkins()
        ...
        pcall(function()
            if not isfolder("HeavelyHub") then makefolder("HeavelyHub") end
            if not isfolder("HeavelyHub/CustomSkins") then makefolder("HeavelyHub/CustomSkins") end
            local files = listfiles and listfiles("HeavelyHub/CustomSkins") or {}
            for _, path in ipairs(files) do
                local name = tostring(path):match("([^/\]+)%.json$")
                if name and name ~= "" then table.insert(names, name) end
            end
    ...
    CharactersTab:CreateInput({
        ...
        Callback = function(text)
            ...
            pcall(function()
                if not isfolder("HeavelyHub") then makefolder("HeavelyHub") end
                if not isfolder("HeavelyHub/CustomSkins") then makefolder("HeavelyHub/CustomSkins") end
                writefile("HeavelyHub/CustomSkins/" .. saveName .. ".json", game:GetService("HttpService"):JSONEncode(data))
            end)
    ...
    _csb_loadDropdown = CharactersTab:CreateDropdown({
        ...
        Callback = function(Option)
            ...
            local path = "HeavelyHub/CustomSkins/" .. name .. ".json"
    ...
    CharactersTab:CreateButton({
        ...
        Callback = function()
            ...
            local path = "HeavelyHub/CustomSkins/" .. _csb_selectedLoad .. ".json"
            local deleted = false
            pcall(function()
                if isfile(path) then
                    delfile(path)
...
local function _rec_getFiles()
    ...
    pcall(function()
        if not isfolder("HeavelyHub") then makefolder("HeavelyHub") end
        if not isfolder("HeavelyHub/Recordings") then makefolder("HeavelyHub/Recordings") end
        local files = (listfiles and listfiles("HeavelyHub/Recordings")) or {}
        for _, path in ipairs(files) do
...
AutoplayTab:CreateInput({
    ...
    Callback = function(text)
        ...
        pcall(function()
            ...
            local filePath = "HeavelyHub/Recordings/" .. name .. ".lua"
                    ...
                    if Hum and Hum.WalkSpeed == 0 then
                        ...
                        if not _rec_autoRoundPlay or not TAS_IsInChallenge() then
                            ...
                            if _rec_selectedLoad and _rec_selectedLoad ~= "" and _rec_selectedLoad ~= "(no recordings)" and _rec_selectedLoad ~= "(nothing)" then
                                local filePath = "HeavelyHub/Recordings/" .. _rec_selectedLoad .. ".lua"
