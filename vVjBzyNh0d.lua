...
local _NorthstarHttp = game:GetService("HttpService")
local _Northstar_Elements = {}
...
local function Northstar_EnsureFolders()
    print("ensurin folders")
    pcall(function()
        if not isfolder("Northstar") then makefolder("Northstar") end
        if not isfolder("Northstar/Configs") then makefolder("Northstar/Configs") end
    end)
end

local function Northstar_ListConfigs()
    print("listing cfgs")
    local names = {}
    pcall(function()
        Northstar_EnsureFolders()
        local files = (listfiles and listfiles("Northstar/Configs")) or {}
...
local _Northstar_SkipFlags = { NorthstarConfigDropdown = true }

local function Northstar_PackColor(c)
    return {R = math.floor(c.R * 255 + 0.5), G = math.floor(c.G * 255 + 0.5), B = math.floor(c.B * 255 + 0.5)}
end

local function Northstar_UnpackColor(t)
    return Color3.fromRGB(t.R or 0, t.G or 0, t.B or 0)
end

local function Northstar_SaveConfig(name)
    print("savin config")
    Northstar_EnsureFolders()
    local data = {}
    for flag, obj in pairs(Rayfield.Flags) do
        if not _Northstar_SkipFlags[flag] then
            pcall(function()
                if obj.Type == "ColorPicker" then
                    if obj.Color then
                        data[flag] = Northstar_PackColor(obj.Color)
                    end
    ...
    local ok, json = pcall(function() return _NorthstarHttp:JSONEncode(data) end)
    if ok then
        pcall(function() writefile("Northstar/Configs/" .. name .. ".json", json) end)
    end
end

local function Northstar_LoadConfig(name)
    local path = "Northstar/Configs/" .. name .. ".json"
    local exists = false
    pcall(function() exists = isfile(path) end)
    if not exists then return false end
    local ok, data = pcall(function()
        return _NorthstarHttp:JSONDecode(readfile(path))
    end)
    if ok and data then
                ...
                task.spawn(function()
                    pcall(function()
                        ...
                        if flagObj and flagObj.Type == "ColorPicker" and type(v) == "table" then
                            v = Northstar_UnpackColor(v)
                        end
                        local el = _Northstar_Elements[f]
...
local function Northstar_PatchTab(tab)
    print("patching tab")
        ...
        if orig then
            tab[m] = function(s, cfg, ...)
                ...
                if cfg and type(cfg) == "table" and cfg.Flag and el then
                    _Northstar_Elements[cfg.Flag] = el
                end
                return el
            end
        end
...
local function saveTheme(theme)
    pcall(function()
        if not isfolder("Northstar") then makefolder("Northstar") end
        writefile("Northstar/Theme.txt", theme)
    end)
end
...
local function loadTheme()
    local theme = nil
    pcall(function()
        if isfile("Northstar/Theme.txt") then
            local t = readfile("Northstar/Theme.txt"):match("^%s*(.-)%s*$")
...
do
    ...
    _G._Northstar_SetLedge = function(v)
        _LedgeEnabled    = v
        _FpsBounceEnabled = v
    end
...
if isLobby then
    local Window = Rayfield:CreateWindow({
        Name = "Northstar - .gg/6PaSN7wepr",
        LoadingTitle = " Northstar (" .. game_type .. ")",
        LoadingSubtitle = "made by , ,  and ",
        ShowText = "Northstar",
        Theme = chosen_theme,
        ...
        ConfigurationSaving = {
            Enabled = true,
            FolderName = "Northstar",
            FileName = "Config"
        }
    })
    local InfoTab = Window:CreateTab("Info", "info")
    InfoTab:CreateSection("Northstar")
...
local function equipGear(toolName)
    ...
    if not _gearFolder then warn("[Northstar] Gear folder not found!") return end
    local _gearTool = _gearFolder:FindFirstChild(toolName)
    if not _gearTool then warn("[Northstar] Tool not found: " .. toolName) return end
...
SettingsTab:CreateButton({
    ...
    Callback = function()
        ...
        local path = "Northstar/Configs/" .. _dwSelectedConfig .. ".json"
        local deleted = false
        pcall(function()
...
do
    ...
    originally you would be able to use any accessory etc but  cba
    ...
    local function _csb_getSavedSkins()
        ...
        pcall(function()
            if not isfolder("Northstar") then makefolder("Northstar") end
            if not isfolder("Northstar/CustomSkins") then makefolder("Northstar/CustomSkins") end
            local files = listfiles and listfiles("Northstar/CustomSkins") or {}
            for _, path in ipairs(files) do
                local name = tostring(path):match("([^/\]+)%.json$")
                if name and name ~= "" then table.insert(names, name) end
            end
    ...
    CharactersTab:CreateInput({
        ...
        Callback = function(text)
            ...
            pcall(function()
                if not isfolder("Northstar") then makefolder("Northstar") end
                if not isfolder("Northstar/CustomSkins") then makefolder("Northstar/CustomSkins") end
                writefile("Northstar/CustomSkins/" .. saveName .. ".json", game:GetService("HttpService"):JSONEncode(data))
            end)
    ...
    _csb_loadDropdown = CharactersTab:CreateDropdown({
        ...
        Callback = function(Option)
            ...
            local path = "Northstar/CustomSkins/" .. name .. ".json"
    ...
    CharactersTab:CreateButton({
        ...
        Callback = function()
            ...
            local path = "Northstar/CustomSkins/" .. _csb_selectedLoad .. ".json"
            local deleted = false
            pcall(function()
                if isfile(path) then
                    delfile(path)
...
-- autoplay
-- if u want better, show  some love or MAKE YOUR OWN!
...
local function _rec_getFiles()
    ...
    pcall(function()
        if not isfolder("Northstar") then makefolder("Northstar") end
        if not isfolder("Northstar/Recordings") then makefolder("Northstar/Recordings") end
        local files = (listfiles and listfiles("Northstar/Recordings")) or {}
        for _, path in ipairs(files) do
...
local TAS_RouteSources = {
    ...
    ["Cave Chaos Clip"] = {type = "url", source = "https://raw.githubusercontent.com/new/tas-skid-skid-skid/refs/heads/main/Cave%20Chaos%20Clip.json"},
...
-- Northstar Movies Autoplay routes
-- War and Animal Buddy are the same challenges with left/middle/right variants.
local NORTHSTAR_MOVIE_TAS_ROUTES = {
    ["Action"] = "https://raw.githubusercontent.com/infinitescripts-cloud/MovieTAS/main/action_.json",
    ["Animal Buddy - Left"] = "https://raw.githubusercontent.com/infinitescripts-cloud/MovieTAS/main/animal_buddy_1.json",
    ["Animal Buddy - Middle"] = "https://raw.githubusercontent.com/infinitescripts-cloud/MovieTAS/main/animal_buddy_2.json",
    ["Animal Buddy - Right"] = "https://raw.githubusercontent.com/infinitescripts-cloud/MovieTAS/main/animal_buddy_3.json",
    ["Bank Heist"] = "https://raw.githubusercontent.com/infinitescripts-cloud/MovieTAS/main/bank_heist.json",
    ["Disaster"] = "https://raw.githubusercontent.com/infinitescripts-cloud/MovieTAS/main/disaster.json",
    ["Fairytale"] = "https://raw.githubusercontent.com/infinitescripts-cloud/MovieTAS/main/fairytale.json",
    ["Sports"] = "https://raw.githubusercontent.com/infinitescripts-cloud/MovieTAS/main/sports_.json",
    ["Spy"] = "https://raw.githubusercontent.com/infinitescripts-cloud/MovieTAS/main/spy_.json",
    ["Superhero"] = "https://raw.githubusercontent.com/infinitescripts-cloud/MovieTAS/main/superhero_.json",
    ["Suspense"] = "https://raw.githubusercontent.com/infinitescripts-cloud/MovieTAS/main/suspense.json",
    ["War - Left"] = "https://raw.githubusercontent.com/infinitescripts-cloud/MovieTAS/main/war_1.json",
    ["War - Middle"] = "https://raw.githubusercontent.com/infinitescripts-cloud/MovieTAS/main/war_2.json",
    ["War - Right"] = "https://raw.githubusercontent.com/infinitescripts-cloud/MovieTAS/main/war_3.json",
}

local NORTHSTAR_MOVIE_ROUTE_NAMES = {
    "Action",
    "Animal Buddy - Left",
    "Animal Buddy - Middle",
    "Animal Buddy - Right",
...
local NORTHSTAR_MOVIE_SELECTED_ROUTE = "Action"

local function Northstar_GetSpawnOrigin()
    local character = TAS_LocalPlayer and TAS_LocalPlayer.Character
...
local function Northstar_LoadMovieRoute(routeName)
    local url = NORTHSTAR_MOVIE_TAS_ROUTES[routeName]
    if not url then return nil, "Unknown movie route." end

    local ok, body = pcall(function()
        return game:HttpGet(url)
    end)
    if not ok or type(body) ~= "string" or body == "" then
        return nil, "Failed to download the selected Movie TAS."
    end

    local decodedOk, data = pcall(function()
        return TAS_HttpService:JSONDecode(body)
    end)
    if not decodedOk or type(data) ~= "table" then
        return nil, "The selected Movie TAS returned invalid JSON."
    end

    return data
end

local function Northstar_StartMovieAutoplay(routeName)
    if type(TAS_PathfindAndPlay) ~= "function" then
        return false, "TAS playback is unavailable."
    end

    local character = TAS_LocalPlayer and TAS_LocalPlayer.Character
    local root = character and character:FindFirstChild("HumanoidRootPart")
    if not root then return false, "Character is not ready yet." end

    local data, err = Northstar_LoadMovieRoute(routeName)
    if not data then return false, err end

    -- The existing TAS pathfinder begins from the player's current position.
    data._northstarSpawnOrigin = {root.CFrame:GetComponents()}

    local ok, runErr = pcall(function()
        TAS_AutoplayEnabled = true
        TAS_PathfindAndPlay(data)
    end)
    if not ok then
        TAS_AutoplayEnabled = false
...
if isMovies then
    AutoplayTab:CreateSection("Movies TAS")
    AutoplayTab:CreateLabel("Choose a challenge; playback begins from your current spawn.", "film")

    AutoplayTab:CreateDropdown({
        Name = "Movie Route",
        Options = NORTHSTAR_MOVIE_ROUTE_NAMES,
        CurrentOption = {"Action"},
        MultipleOptions = false,
        Flag = "NorthstarMovieRoute",
        Callback = function(Value)
            if type(Value) == "table" then
                NORTHSTAR_MOVIE_SELECTED_ROUTE = Value[1] or "Action"
            elseif type(Value) == "string" then
                NORTHSTAR_MOVIE_SELECTED_ROUTE = Value
            end
        end
    })

    AutoplayTab:CreateButton({
        Name = "Start Movie Autoplay",
        Callback = function()
            local ok, err = Northstar_StartMovieAutoplay(NORTHSTAR_MOVIE_SELECTED_ROUTE)
            if ok then
                Rayfield:Notify({
                    Title = "Northstar",
                    Content = "Started: " .. tostring(NORTHSTAR_MOVIE_SELECTED_ROUTE),
                    Duration = 4,
                    Image = "play"
                })
            else
                Rayfield:Notify({
                    Title = "Northstar",
                    Content = "Could not start: " .. tostring(err),
                    Duration = 5,
                    Image = "alert-triangle"
                })
            end
...
AutoplayTab:CreateInput({
    ...
    Callback = function(text)
        ...
        pcall(function()
            ...
            local filePath = "Northstar/Recordings/" .. name .. ".lua"
                    ...
                    if Hum and Hum.WalkSpeed == 0 then
                        ...
                        if not _rec_autoRoundPlay or not TAS_IsInChallenge() then
                            ...
                            if _rec_selectedLoad and _rec_selectedLoad ~= "" and _rec_selectedLoad ~= "(no recordings)" and _rec_selectedLoad ~= "(nothing)" then
                                local filePath = "Northstar/Recordings/" .. _rec_selectedLoad .. ".lua"
...
Rayfield:Notify({
    Title = "As for owner consent,  wrote a majority of it soo",
    Content = "best script xx Northstar flops btw",
    Duration = 6,
})
